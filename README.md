...
The bar stays compact: each account is reduced to its initial, selected marker, core Codex quota percentage, and pace/state symbol (for example `●W:11%↑ · P:!`). Hovering opens a grouped mini-dashboard with full aliases, readiness, the core Codex quota bar, remaining capacity, reset time, and an outcome-oriented forecast such as `empty in ~3h 20m` or `survives to reset`. Locally recorded request and token totals use decimal units such as `1.2k`, `3.4M`, and `5.6B`; exact integers remain available in the status API. Additional feature meters are retained in the status API but omitted from Waybar. CSS classes include `codex-pool` plus health/pace classes from the returned payload.

The tooltip also reports an **API-equivalent estimate**, calculated with [OpenAI Standard API token pricing](https://developers.openai.com/api/docs/pricing) verified on 2026-07-22. It is a hypothetical comparison, not a ChatGPT subscription charge or bill. Recognized GPT-5.6 text-token usage is normalized to Standard service-tier rates; regional uplifts, separately billed tools, and unknown internal model names are excluded, with request coverage shown beside the result. Existing database rows predate this daemon's [cache-write accounting](https://developers.openai.com/api/docs/guides/prompt-caching#requirements), so upgraded databases display an honest lower–upper range for eligible history instead of a falsely exact total. History pruned before the upgrade remains unpriced and lowers the reported coverage. Historical estimates are filled in resumable background batches after the daemon starts accepting connections.
